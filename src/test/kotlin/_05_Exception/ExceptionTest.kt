...
class ExceptionTest: DescribeSpec ({
    ...
    describe("상속 관계인 예외 클래스") {
        ...
        context("자식 클래스 is 부모 클래스는 참이다.") {
            (child1 is CommonException) shouldBeEqual true
            (child2 is FirstLevelException1) shouldBeEqual true
            (child2 is CommonException) shouldBeEqual true
//            (other is CommonException) shouldBeEqual false // 컴파일 에러
        }
    ...
    describe("StringToInt 클래스는") {
        ...
        context("한글을 정수로 변환을 시도한다면") {
            ...
            it("returnZero 함수는 0을 반환한다.") {
                str.returnZero() shouldBeEqual 0
            }
        }

        context("숫자 문자열을 정수로 변환을 시도한다면") {
            it("정수를 반환한다.") {
                number.throwException() shouldBeEqual 1234
                number.returnZero() shouldBeEqual 1234
            }
        }
    }
    ...
    describe("finally 블록에서 return문을 선언한다면") {
        ...
        it("다른 return문은 무시된다.") {
            exception.func1() shouldBeEqual "finally"
            exception.func2() shouldBeEqual "finally"
            exception.func3() shouldBeEqual "finally"
        }
    }
